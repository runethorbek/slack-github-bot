...
For the Slice 1 private-DM tracer, an authorized root DM is itself the root of a new conversation. The bot replies beneath it with a deterministic acknowledgement; DM history is not loaded and Gemini is not invoked.
...
The single Slack member ID authorized to use the Slice 1 direct-message tracer. Store it as a GitHub Actions repository secret. In Slack, open the member's profile, choose *More*, and select *Copy member ID*; use the ID value (typically beginning with `U`), not the display name.
